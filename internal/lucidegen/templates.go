...
{{if .IsFirstFile}}// mergeClasses combines the base "icon" class with any additional classes from attrs and class string
func mergeClasses(class string, attrs templ.Attributes) templ.Attributes {
	if attrs == nil {
	...
	if existingClass, hasClass := attrs["class"]; hasClass {
		if classStr, ok := existingClass.(string); ok && classStr != "" {
			finalClass += " " + classStr
		}
	}
	
	merged["class"] = finalClass
	return merged
}

// mergeSVGAttrs merges class string with SVG attributes, copying to avoid modifying original
func mergeSVGAttrs(class string, attrs templ.Attributes) templ.Attributes {
	if attrs == nil {
		attrs = templ.Attributes{}
	}
	...
	// Add or merge class
	if class != "" {
		if existingClass, hasClass := merged["class"]; hasClass {
			if classStr, ok := existingClass.(string); ok && classStr != "" {
				merged["class"] = class + " " + classStr
			} else {
				merged["class"] = class
			}
		} else {
...
{{end}}{{range .Icons}}
// {{.FuncName}}Templ renders the {{.Name}} Lucide icon wrapped in a span (internal templ component)
templ {{.FuncName}}Templ(class string) {
	<span { mergeClasses(class, templ.Attributes{})... }>
		<svg viewBox="{{.ViewBox}}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
			{{.Content}}
		</svg>
	</span>
}

// {{.FuncName}} renders the {{.Name}} Lucide icon wrapped in a span
// Category: {{.Category}}
func {{.FuncName}}(class ...string) templ.Component {
	classStr := ""
	if len(class) > 0 {
		classStr = class[0]
	}
	return {{.FuncName}}Templ(classStr)
}

// {{.FuncName}}WithAttrs renders the {{.Name}} Lucide icon wrapped in a span with attributes
// Category: {{.Category}}
templ {{.FuncName}}WithAttrs(class string, attrs templ.Attributes) {
	<span { mergeClasses(class, attrs)... }>
		<svg viewBox="{{.ViewBox}}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
			{{.Content}}
		</svg>
	</span>
...
// {{.FuncName}}SVGTempl renders the raw {{.Name}} Lucide icon SVG (internal templ component)
templ {{.FuncName}}SVGTempl(class string) {
	if class != "" {
		<svg viewBox="{{.ViewBox}}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class={ class }>
			{{.Content}}
		</svg>
	} else {
		<svg viewBox="{{.ViewBox}}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
// {{.FuncName}}SVG renders the raw {{.Name}} Lucide icon SVG
// Category: {{.Category}}
func {{.FuncName}}SVG(class ...string) templ.Component {
	classStr := ""
	if len(class) > 0 {
		classStr = class[0]
	}
	return {{.FuncName}}SVGTempl(classStr)
}

// {{.FuncName}}SVGWithAttrs renders the raw {{.Name}} Lucide icon SVG with attributes
// Category: {{.Category}}
templ {{.FuncName}}SVGWithAttrs(class string, attrs templ.Attributes) {
	<svg viewBox="{{.ViewBox}}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" { mergeSVGAttrs(class, attrs)... }>
		{{.Content}}
	</svg>
}
{{end}}`
...
func IconWithAttrs(name IconName, class string, attrs templ.Attributes) templ.Component {
	switch name {
{{range .Icons}}	case {{call $.ToConstantName .Name $.Prefix}}:
		return {{.FuncName}}WithAttrs(class, attrs)
{{end}}	default:
		return nil
	}
}
...
func IconSVGWithAttrs(name IconName, class string, attrs templ.Attributes) templ.Component {
	switch name {
{{range .Icons}}	case {{call $.ToConstantName .Name $.Prefix}}:
		return {{.FuncName}}SVGWithAttrs(class, attrs)
{{end}}	default:
		return nil
	}
}
...
type TemplateData struct {
	...
	IsFirstFile    bool // Indicates if this is the first icons file (for mergeClasses)
}
...
func generateIconsFile(icons []IconData, config Config, outputPath string) error {
	return generateIconsFileWithIndex(icons, config, outputPath, true)
}

// generateIconsFileWithIndex creates an icons template file with index control
func generateIconsFileWithIndex(icons []IconData, config Config, outputPath string, isFirstFile bool) error {
	data := TemplateData{
