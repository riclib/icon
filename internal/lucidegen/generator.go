...
func generateFiles(icons []IconData, config Config) ([]string, error) {
	...
	// Clean up any existing icon files first
	existingFiles, _ := filepath.Glob(filepath.Join(config.OutputDir, "icons*.templ"))
	for _, f := range existingFiles {
		os.Remove(f)
	}
	
	// Generate icons files (split into chunks of 1000)
	const iconsPerFile = 1000
	numFiles := (len(icons) + iconsPerFile - 1) / iconsPerFile
	
	for i := 0; i < numFiles; i++ {
		start := i * iconsPerFile
		end := start + iconsPerFile
		if end > len(icons) {
			end = len(icons)
		}
		
		chunk := icons[start:end]
		fileName := fmt.Sprintf("icons-%03d.templ", i+1)
		iconsFile := filepath.Join(config.OutputDir, fileName)
		
		if err := generateIconsFileWithIndex(chunk, config, iconsFile, i == 0); err != nil {
			return nil, fmt.Errorf("failed to generate icons file %s: %w", fileName, err)
		}
		createdFiles = append(createdFiles, iconsFile)
	}

	// Generate registry file
	registryFile := filepath.Join(config.OutputDir, "registry.go")
